...
int main(int argc, char* argv[]) {//argv[1] : CLIENT1 Port  argv[2] : CLIENT2 Port
	...
	char press_msg[5],brightness_msg[5],weight_msg[5];
	int press = 0;
	int touch = 0;
	int brightness = 0;
	int weight = 0;
	int fd1 = open(DEVICE, O_RDWR); //압력센서
	int fd2 = open(DEVICE, O_RDWR); //조도센서	
	if (prepare(fd1) == -1) return -1;
	if (prepare(fd2) == -1) return -1;
	if (-1 == GPIOExport(TOUCH_PIN)) return(1);
	if (-1 == GPIODirection(TOUCH_PIN, IN)) return(2);
	if (argc != 3) printf("Usage : %s <port1> <port2>\n", argv[0]); //SERVER_IP <CLIENT1 Port> <CLIENT2 Port>
	serv_sock = socket(PF_INET, SOCK_STREAM, 0);
	if (serv_sock == -1) error_handling("socket() error");
	memset(&serv_addr, 0, sizeof(serv_addr));
	serv_addr.sin_family = AF_INET;
	serv_addr.sin_addr.s_addr = htonl(INADDR_ANY);
	serv_addr.sin_port = htons(atoi(argv[1])); //CLIENT1과 연결
	if (bind(serv_sock, (struct sockaddr*) & serv_addr, sizeof(serv_addr)) == -1)
		error_handling("bind() error");
	while (1)
	{
		touch = GPIORead(TOUCH_PIN);
		press = readadc(fd1, PRESS_CH1);
		brightness = readadc(fd2, BRIGHTNESS_CH);
		if (touch == 1){
			if (clnt_sock1 < 0) {
			...
			snprintf(press_msg, 5, "%d", press);
			write(clnt_sock1, press_msg, sizeof(press_msg));
			snprintf(brightness_msg, 5, "%d", brightness);
			write(clnt_sock1, brightness_msg, sizeof(brightness_msg));
			close(clnt_sock1);
			clnt_sock1 = -1;
		}
		else	break;
	...
	serv_addr.sin_family = AF_INET;
	serv_addr.sin_addr.s_addr = htonl(INADDR_ANY);
	serv_addr.sin_port = htons(atoi(argv[2])); //CLIENT2와 연결
	if (bind(serv_sock, (struct sockaddr*) & serv_addr, sizeof(serv_addr)) == -1)
		error_handling("bind() error");
	while (1)
	{		
		weight = readadc(fd1, PRESS_CH2);
		if (clnt_sock2 < 0) {
			if (listen(serv_sock, 5) == -1) error_handling("listen() error");
			clnt_addr_size2 = sizeof(clnt_addr2);
			clnt_sock2 = accept(serv_sock, (struct sockaddr*) & clnt_addr2, &clnt_addr_size2);
			if (clnt_sock2 == -1) error_handling("accept() error");
		}
		
		snprintf(weight_msg, 5, "%d", weight);
		write(clnt_sock2, weight_msg, sizeof(weight_msg));
		close(clnt_sock2);
		clnt_sock2 = -1;
	...
	close(serv_sock);
	if (-1 == GPIOUnexport(TOUCH_PIN))	return(4);
	return(0);
}
